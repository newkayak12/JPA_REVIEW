...
//            원래는 Object[]로 받아서 형변환을 했지만 다른 컬럼이 없는 관계로 이렇게 했다.
    ...
    public static void multiEntity(){
        ...
        for(Object[] row : resultList){
            Member_jpql2 ms = (Member_jpql2) row[0];
            Product_jpql ps = (Product_jpql) row[1];
            Address_jpql as = (Address_jpql) row[2];
            System.out.println(ms + " : "+ ps+ " : "+as);
//물론 이렇게 조회된 엔티티도 영속성 컨텍스트에서 관리된다.
        }
    }
}
